...
#>
    ...
    function Validate-SoftwareInstallation {
        ...
        Begin {
            Write-IntuneDetectionScriptLog -Message "Starting validation for $SoftwareName" -Level "NOTICE"
        ...
        Process {
            while ($retryCount -lt $MaxRetries -and -not $isInstalled) {
                $retryCount++
                Write-IntuneDetectionScriptLog -Message "Attempt $retryCount of $MaxRetries to validate $SoftwareName installation." -Level "INFO"
                ...
                if ($installResult.IsInstalled) {
                    ...
                    Write-IntuneDetectionScriptLog -Message "$SoftwareName is installed. Detected version: $installedVersion" -Level "INFO"
                    break
                }
                else {
                    Write-IntuneDetectionScriptLog -Message "Validation attempt $retryCount failed: $SoftwareName not found. Retrying in $DelayBetweenRetries seconds..." -Level "WARNING"
                    Start-Sleep -Seconds $DelayBetweenRetries
                }
            }
    
            if (-not $isInstalled) {
                Write-IntuneDetectionScriptLog -Message "$SoftwareName is not installed after $MaxRetries retries." -Level "ERROR"
                return $result
            }
    
            # Use the Compare-SoftwareVersion function to compare versions
            Write-IntuneDetectionScriptLog -Message "Comparing installed version of $SoftwareName with minimum and latest versions." -Level "INFO"
            ...
            Write-IntuneDetectionScriptLog -Message "Validation complete for $SoftwareName. Meets minimum requirements: $($result.MeetsMinRequirement). Is up-to-date: $($result.IsUpToDate)." -Level "INFO"
