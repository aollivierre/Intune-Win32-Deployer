...
_____       _                      _____           _        _ _             _    _ _   _ _ _ _         
|_   _|     | |                    |_   _|         | |      | | |           | |  | | | (_) (_) |        
  | |  _ __ | |_ _   _ _ __   ___    | |  _ __  ___| |_ __ _| | | ___ _ __  | |  | | |_ _| |_| |_ _   _ 
  | | | '_ \| __| | | | '_ \ / _ \   | | | '_ \/ __| __/ _` | | |/ _ \ '__| | |  | | __| | | | __| | | |
 _| |_| | | | |_| |_| | | | |  __/  _| |_| | | \__ \ || (_| | | |  __/ |    | |__| | |_| | | | |_| |_| |
|_____|_| |_|\__|\__,_|_| |_|\___| |_____|_| |_|___/\__\__,_|_|_|\___|_|     \____/ \__|_|_|_|\__|\__, |
                                                                                                   __/ |
                                                                                                  |___/ 
...
Write-Host "`nIntune Installer Utility v1.0" -ForegroundColor Green
...
function Write-IntuneInstallerLog {
    param (
        [string]$Message,
        [string]$Level = "INFO"
    ...
    $logFilePath = [System.IO.Path]::Combine($env:TEMP, 'InstallInstaller.log')
...
function Reset-ModulePaths {
    ...
    begin {
        ...
        Write-IntuneInstallerLog -Message "Initializing Reset-ModulePaths function..." -Level "DEBUG"
    }

    process {
        try {
            # Log the start of the process
            Write-IntuneInstallerLog -Message "Resetting module paths to default values..." -Level "INFO"
            ...
            Write-IntuneInstallerLog -Message "PSModulePath successfully set to: $($env:PSModulePath -split ';' | Out-String)" -Level "INFO"
            ...
            Write-IntuneInstallerLog -Message "PSModulePath environment variable set for the current user." -Level "INFO"
        }
        catch {
            # Capture and log any errors that occur during the process
            $errorMessage = $_.Exception.Message
            Write-IntuneInstallerLog -Message "Error resetting module paths: $errorMessage" -Level "ERROR"
        ...
        Write-IntuneInstallerLog -Message "Reset-ModulePaths function completed." -Level "DEBUG"
    }
}
...
switch ($global:mode) {
    'dev' {
        Write-IntuneInstallerLog -Message "Running in development mode" -Level 'Warning'
        # Your development logic here
    }
    'prod' {
        Write-IntuneInstallerLog -Message "Running in production mode" -Level 'INFO'
        # Your production logic here
    }
    default {
        Write-IntuneInstallerLog -Message "Unknown mode. Defaulting to production." -Level 'ERROR'
...
while (-not $acquiredLock -and $attempt -lt $maxAttempts) {
    $attempt++
    Write-IntuneInstallerLog -Message "Attempt $attempt to acquire the lock..."
    ...
    if (-not $acquiredLock) {
        ...
        Write-IntuneInstallerLog "Failed to acquire the lock. Retrying in $initialWaitTime seconds..." -Level 'WARNING'
...
try {
    if ($acquiredLock) {
        Write-IntuneInstallerLog -Message "Acquired the lock. Proceeding with module installation and import."
        ...
        if ($global:mode -eq "dev") {
            ...
            Write-IntuneInstallerLog -Message "Running in dev mode. Importing module from local path."
            Import-Module 'C:\code\ModulesV2\EnhancedModuleStarterAO\EnhancedModuleStarterAO.psm1'
        }
        elseif ($global:mode -eq "prod") {
            # In prod mode, execute the script from the URL
            Write-IntuneInstallerLog -Message "Running in prod mode. Executing the script from the remote URL."
            # Invoke-Expression (Invoke-RestMethod "https://raw.githubusercontent.com/aollivierre/module-starter/main/Install-EnhancedModuleStarterAO.ps1")


            # Check if running in PowerShell 5
            if ($PSVersionTable.PSVersion.Major -ne 5) {
                Write-IntuneInstallerLog -Message "Not running in PowerShell 5. Relaunching the command with PowerShell 5."
                ...
                Write-IntuneInstallerLog -Message "Running in PowerShell 5. Executing the command."
                Invoke-Expression (Invoke-RestMethod "https://raw.githubusercontent.com/aollivierre/module-starter/main/Install-EnhancedModuleStarterAO.ps1")
            }
            ...
            Write-IntuneInstallerLog -Message "Invalid mode specified. Please set the mode to either 'dev' or 'prod'." -Level 'WARNING'
            exit 1
        }
        ...
        # Check if running in PowerShell 5
        if ($PSVersionTable.PSVersion.Major -ne 5) {
            Write-IntuneInstallerLog -Message  "Not running in PowerShell 5. Relaunching the function call with PowerShell 5."
            ...
            Write-IntuneInstallerLog -Message "Running in PowerShell 5. Executing Invoke-ModuleStarter."
            Invoke-ModuleStarter @moduleStarterParams
        }
        ...
        Write-IntuneInstallerLog -Message "Critical section execution time: $timeTaken seconds"
        ...
        Write-IntuneInstallerLog -Message "Module installation and import completed."
    }
    else {
    ...
    if ($acquiredLock) {
        $mutex.ReleaseMutex()
        Write-IntuneInstallerLog -Message "Released the lock."
    }
...
try {
    ...
    $ValidatePSADTFilesParams = @{
        ScriptRoot = "$PSScriptRoot\PSAppDeployToolkit_3.10.2\Toolkit"
    }
    ...
    # Start-Process -FilePath "$Psscriptroot\PSAppDeployToolkit_3.10.2\Toolkit\Deploy-Application.exe" -Wait -WindowStyle Hidden


    
    # Define the parameters for Start-Process
    $processParams = @{
        FilePath     = "$PSScriptRoot\PSAppDeployToolkit_3.10.2\Toolkit\Deploy-Application.exe"
        ArgumentList = '-DeploymentType "install" -DeployMode "Silent"'
        Wait         = $false
        WindowStyle  = 'Hidden'
    }

    # Execute the Start-Process command with splatting
