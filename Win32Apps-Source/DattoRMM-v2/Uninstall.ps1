...
try {
    ...
    # Start-Process -FilePath "$PSScriptRoot\PSAppDeployToolkit_3.10.2\Toolkit\Deploy-Application.exe" -ArgumentList "-DeploymentType `"Uninstall`" -DeployMode `"Silent`"" -Wait -WindowStyle Hidden


    # Define the parameters for Start-Process
    $processParams = @{
        FilePath     = "$PSScriptRoot\PSAppDeployToolkit_3.10.2\Toolkit\Deploy-Application.exe"
        ArgumentList = '-DeploymentType "Uninstall" -DeployMode "Silent"'
        Wait         = $false
        WindowStyle  = 'Hidden'
    }
